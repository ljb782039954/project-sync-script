...
- **.syncScript_logs**：用于记录同步日志（JSON格式）
  - 源项目日志：`源项目根目录/.syncScript_logs/sync_目标项目名_YYYYMMDD_HHMMSS.json`
  - 目标项目日志：`目标项目根目录/.syncScript_logs/sync_YYYYMMDD_HHMMSS.json`
  - 统计日志：`目标项目根目录/.syncScript_logs/total_sync_log.json`（汇总所有同步记录）
  - 每次同步会在源项目和目标项目各生成一份 JSON 日志文件，并更新统计日志
...
```json
{
  "timestamp": "2025-12-16T15:45:51.123Z",
  ...
  "files": {
    "added": ["src/components/Button.js", "src/utils/helper.js"],
    "modified": ["README.md"],
    "deleted": []
  },
  "excluded_files": ["node_modules/package.json"],
  "duration_ms": 1250,
  "status": "success",
  "errors": []
}
...
#### 统计日志 (total_sync_log.json)
```json
{
  "last_updated": "2025-12-16T15:45:51.123Z",
  "total_syncs": 15,
  "summary": {
    "total_files_added": 45,
  ...
  "recent_syncs": [
    {
      ...
      "sync_mode": "incremental",
      "target_dir": "/path/to/target",
      "statistics": {"added": 2, "modified": 1, "deleted": 0},
      "status": "success"
    }
  ]
}
